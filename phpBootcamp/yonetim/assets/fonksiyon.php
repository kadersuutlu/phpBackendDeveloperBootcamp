...
class yonetim 
{
    ...
    function siteayar($baglanti) {
        ...
        if($_POST):
            ...
            title=?,metatitle=?,metadesc=?,metakey=?,metaauthor=?,metaowner=?,metacopy=?,logoyazisi=?,link=?,
            git=?,ints=?,telefonno=?,adres=?,mailadres=?,slogan=?,referansbaslik=?,galeribaslik=?,
            ...
            $guncelleme->bindParam(9,$link,PDO::PARAM_STR);
            $guncelleme->bindParam(10,$git,PDO::PARAM_STR);
      ...
      <form action="control.php?sayfa=siteayar" method="post">
        ...
        <div class="row">
        <div class="col-lg-3 border-right pt-3 text-left">
        <span id="siteayarfont">Github</span>
        </div>
        <div class="col-lg-9 p-1">
        <input type="text" name="git" class="form-control" value="<?php echo $sonuc["git"];?>"   />
        </div>
        </div>
        </div>
        <!--ara--> 
